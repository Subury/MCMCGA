    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == len(iterable):
                eta_seconds = iter_time.global_avg * len(iterable)
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    output_dir = Path(f'{args.output_dir}/weights')
    output_dir.mkdir(parents=True, exist_ok=True)
    epoch_name = str(epoch).zfill(len(str(args.epochs)))
    if loss_scaler is not None:
        ...
        for checkpoint_path in checkpoint_paths:
            ...
            save_on_master(to_save, output_dir / 'lastest.pth')
    else:
    ...
    if args.resume:
        if args.resume.startswith('https'):
            ...
            for i in list(checkpoint['model'].keys()):
                if i not in model_without_ddp.state_dict():
